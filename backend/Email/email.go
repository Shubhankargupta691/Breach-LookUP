...
type InputData struct {
	Input string `json:"input"` // Field for user input (email)
}

type ResponseData struct {
	Message string `json:"message"` // Response message
	Email   string `json:"email"`   // Received email
}
...
func HandleInput(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method == http.MethodPost {
		var data InputData
		err := json.NewDecoder(r.Body).Decode(&data) // Decode the JSON body
		if err != nil {
		...
		// Respond with the message and received email
